...
//cosine_filter_transpose(f_imData, filter_bank, height, width, filter_dim, filter_dim, num_filters, item->ind->data, item->val->data, item->val->pitch);
 ...
 * Filter 2 cpu
 * *************************/
 float * block_histogram(float* his,float* ind,float* val, int max_bin, int cell_size, int start_x, int start_y, int im_height, int im_width) {

     //variables
     float * id_data = ind;//ind->get_HOST_PTR(BUF_READ);
     float * wt_data = val;//val->get_HOST_PTR(BUF_READ);
 ...
 //	float * out_data = (float *) malloc(n_parts_x * n_parts_y * max_bin * sizeof(float)); // _mm_malloc()
     ...
     float* his = new float(n_parts_x_gpu * n_parts_y_gpu * 100); //= (float *) malloc(his->size);

     for (int write_i=0; write_i<n_parts_y; write_i++) {
         for (int write_j=0; write_j<n_parts_x; write_j++) {
             int out_ind = (write_i*n_parts_x + write_j) * pitch/sizeof(float);//his->pitch/sizeof(float);
             int read_i = (start_i + (write_i * cell_size)) * pitch/sizeof(float);//ind->pitch/sizeof(float);
             for (int i=0; i<cell_size; i++) {
                 ...
                 for (int j=0; j<cell_size; j++) {
                     ...
                     his[out_ind + bin_ind] += weight;
                 }
                 read_i += pitch/sizeof(float);//ind->pitch/sizeof(float);
             }
         }
     }
     return his;
 ...
 * Filter 3 CPU
 * ***************************/
 float * pwdist_c(float* a_data,float* b_data) {

     float *ptra = a_data;//a_data->get_HOST_PTR(BUF_READ); // should receive de pointer and then use it. now ptra = a_data->data
     float *ptrb = b_data;//b_data->get_HOST_PTR(BUF_READ);
     float * out_data= new float(height*width/8*height/8);//out->get_HOST_PTR(BUF_WRITE);

     //cerr << "A: height = " << aheight << " width = " << awidth << " B: height = " << bheight << " width " << bwidth << endl;
     //  float * out_data = out->data;
     int owidth = pitch/sizeof(float);//out->pitch/sizeof(float);
     int aheight=height;//a_data->height;
     int awidth=pitch/sizeof(float);//a_data->pitch/sizeof(float);
     int bheight=height;//b_data->height;
